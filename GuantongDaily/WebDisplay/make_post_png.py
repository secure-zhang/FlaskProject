...
class YuMi():
    ...
    def run(self,ymkqjc1, ymkqjc2, dfkqjc1, dfkqjc2):
        self.is_save_dir()
        tid_item = {
            'c':{'1':'S0264478','5':'S0264479','9':'S0264480'},
            'cs':{'1':'S0264484','5':'S0264485','9':'S0264486'},
        }
        ymkqjc1 = tid_item[ymkqjc1.split('-')[0]][ymkqjc1.split('-')[1]]
        ymkqjc2 = tid_item[ymkqjc2.split('-')[0]][ymkqjc2.split('-')[1]]

        param1_item = dict(iter(self.select_target_datas(ymkqjc1)))
        param2_item = dict(iter(self.select_target_datas(ymkqjc2)))
        data_list = [(k,v-param2_item[k]) for k,v in param1_item.items() if k in param2_item]

        param = self.format_data(data_list)
        self.kpztljc(param,self.save_path%'ymkqjc')

        dfkqjc1 = tid_item[dfkqjc1.split('-')[0]][dfkqjc1.split('-')[1]]
        dfkqjc2 = tid_item[dfkqjc2.split('-')[0]][dfkqjc2.split('-')[1]]
        param1_item = dict(iter(self.select_target_datas(dfkqjc1)))
        param2_item = dict(iter(self.select_target_datas(dfkqjc2)))
        data_list = [(k, v / param2_item[k]) for k, v in param1_item.items() if k in param2_item]
        param = self.format_data(data_list)
        self.kpztljc(param,self.save_path%'dfkqjc')


if __name__ == '__main__':
    yz = YuMi()
    yz.run('c-1','c-1','cs-1','cs-5')